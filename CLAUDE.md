...
- **A red `npm run deploy` does not mean the deploy failed.** npm's lifecycle fires the `postdeploy` script afterwards — [scripts/migrate-baseline.js](scripts/migrate-baseline.js), which was written for Railway and runs `prisma migrate deploy` against whatever `DATABASE_URL` the environment/`.env` provides. It runs **after** the extension version is already released, and it fails hard when no DB is reachable. Read the Shopify CLI output above it before concluding anything: **retrying releases a second version.** (Pre-existing behaviour, not introduced by the minify wrapper.)
...
- `npm run deploy -- -c dev --allow-updates` (or `-c prod`) — **the only correct way to deploy.** Minify-then-`shopify app deploy` wrapper ([scripts/deploy-minified.mjs](scripts/deploy-minified.mjs)); all args pass through. Calling `shopify app deploy` directly fails on the 100 KiB Liquid limit — see both deploy gotchas above, including why a non-zero exit code here may still be a successful release
