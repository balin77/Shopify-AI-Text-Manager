...
2. **Bei ausgeschaltetem Embed — dem Zustand jedes Händlers, der gerade einschalten will — kam die Warnung aus §1.2 Zeile 2 nie.** `appEmbedDetected` ist nur dann `true`, wenn irgendeine Seite das Merkmal trug; ist das Embed aus, kann keine es tragen, also ist `originKnown` dauerhaft falsch und jeder theme-ausgelieferte Typ landete auf `originUnknown` statt `foreignOnly` — mit Ton `info` und einem Rat („starte einen neuen Crawl"), den kein Crawl einlösen kann. Genau der Händler aus der Auslöser-Geschichte dieses Plans sah damit den mildesten Zustand. Zuerst behoben, ohne etwas zu behaupten, was die Daten nicht hergeben (`originUnknown` nannte beide Ursachen und gab den Schluss, der unter beiden gilt). **Danach durch eine Produktentscheidung des Eigentümers ersetzt** (2026-08-19, `UNMARKED_COUNTS_AS_FOREIGN`): unmarkiertes Markup wird einer **fremden Quelle** zugeschrieben — dem Theme ODER einer anderen App — statt als „Herkunft unklar" gemeldet zu werden. Die zweite Möglichkeit ist keine Fußnote: Markup kommt auf einem Shopify-Shop mindestens so oft aus einer zweiten App wie aus dem Theme, und wer „im Theme beheben" liest, sucht an der einen Stelle, an der es nicht liegt. Die Beweislage hat sich nicht geändert — die Einschätzung, welche der beiden Ursachen wahrscheinlich ist, schon: Die SEO-Sektion ging vor Tagen produktiv, es gibt fast keine Installationen, und „Herkunft unklar" wäre der Zustand gewesen, den nahezu jeder Händler zu sehen bekommt. Doppeltes Markup kann daraus nicht entstehen — unter **beiden** Ursachen liegt der Typ bereits auf der Seite, „nicht einschalten" gilt so oder so; falsch werden kann nur die Zuschreibung, was die Sektion in einer Zeile einräumt, sobald kein Merkmal gefunden wurde. Das Urteil bleibt in Typ, Gate und Tests, damit die Rücknahme eine Konstante ist.
...
**Entschieden (2026-08-19), nachdem der Reviewer es aufgeworfen hatte:** Die Galerie-`VideoObject`s werden **weiter ausgegeben**, auch ohne `uploadDate`. Der Vorschlag war, sie nur bei gesetztem `custom.video_upload_date` zu emittieren, weil die Eigenschaft bei Google REQUIRED ist und ein Eintrag ohne sie in der Search Console als ungültig erscheint — auf einem Schalter mit `default: true`. Drei Punkte gegen die Verschärfung:

- §0.4s Lehre („ungültiges Markup ist schlimmer als keines") trägt hier **nicht**. Dort ging es um zwei `Product`-Knoten mit identischer `@id`, die einen funktionierenden Eintrag korrumpieren. Ein unvollständiges `VideoObject` steht in einem eigenen Script-Tag und beschädigt nichts; es kostet eine Zeile im Enhancement-Report, kein Ranking.
- Die Varianten-Galerie ist eine **JS-Insel**, die ihre Videos aus einem Metafeld liest. Ob Googlebot das Video im gerenderten HTML überhaupt sieht, ist unbekannt — dann wäre unser JSON-LD das einzige Signal, dass es existiert. Nicht ausgeben heißt garantiert null; ausgeben heißt im schlechtesten Fall eine Fehlerzeile. (Ob Google einen Eintrag ohne Pflichtfeld für die Auffindung verwertet, ist **nicht gemessen** und wird hier auch nicht behauptet.)
- Der eigentliche Hebel ist nicht *ob*, sondern *dass es jemand erfährt*: der Fix ist ein Datums-Metafeld pro Produkt.

**Was daraufhin gebaut wurde:** Der Hinweis stand bis dahin in einem 700-Zeichen-Absatz in Grau, verklebt mit dem FAQ-Hinweis, und war generisch — er erschien auch bei Shops ohne Galerie-Videos. Er ist jetzt eine eigene Info-Box mit der Folge zuerst („Galerie-Videos brauchen ein Datum von dir") und dem Metafeldnamen fett; der Video- und der FAQ-Hinweis stehen getrennt daneben. Der `enable_video`-Info-Text im Theme-Editor sagt die Folge ebenfalls im ersten Drittel statt am Ende.

### 5.6 Der Galerie-Video-Befund (2026-08-19)

Die im vorigen Absatz als offen notierte Erkennung ist gebaut. [gallery-video-audit.server.ts](../../app/services/seo/gallery-video-audit.server.ts) beantwortet: welche Produkte haben ein Video, das **nur** in einer Varianten-Galerie liegt, und welche davon können überhaupt ein Rich Result erzeugen.

- **Live-Sweep, kein Cache-Lesen.** Die beiden Metafelder sind nirgends gespiegelt — `ProductVariant.galleryJson` hält Datei-GIDs, eine externe URL hinterlässt im Cache keine Spur. Sie zu cachen hieße: eine Spalte auf **allen vier** Produkt-Schreibpfaden plus ein Diskriminator für Zeilen davor. Ein begrenzter Sweep in einem ohnehin detachten Task ist die billigere ehrliche Antwort.
- **Flach über `productVariants`, nicht verschachtelt in `products`.** Eine verschachtelte Connection multipliziert die Query-Kosten mit der Seitengröße des Elters und throttelt jeden Katalog von Belang. 100 Varianten × 2 Metafelder je Seite, hart begrenzt auf 20 Seiten.
- **Er hängt am bestehenden Knopf.** Kein zweiter Task-Typ, kein zweiter Polling-Zweig: `seoJsonLdAudit` führt ihn nach dem DB-Scan aus und legt das Ergebnis unter `galleryVideos` in dasselbe `Task.result`. `json-ld-audit.service.ts` bleibt dokumentiert DB-only und ohne `admin`-Mock testbar.
- **Zwei Zahlen, die nicht zusammengelegt werden dürfen.** Ein YouTube-Video ohne Datum ist ein Metafeld von einem Rich Result entfernt; ein **Vimeo**-Galerie-Video erzeugt gar kein Markup (kein ableitbares Vorschaubild), da hilft kein Datum. Als eine Zahl gemeldet, schickt man Händler dazu, ein Datum zu setzen, das nichts ändert.
- **Drei Nichtwissen-Zustände, alle „nicht geprüft" und keiner „nichts gefunden":** ein Task-Ergebnis von vor dieser Funktion trägt den Schlüssel gar nicht; ein Sweep, der geworfen hat, gibt `null`; und ein Sweep, der abgelehnt wurde, **bevor** er eine einzige Variante gelesen hat, gibt ebenfalls `null` — eine Null dort hätte als „keine Galerie-Videos gefunden" gerendert, ein sicherer Falschbefund aus einer Abfrage, die nie lief. Nur ein Teil-Sweep behält seine Zahlen, mit `capped` daneben.
- **KEIN neuer Parser.** Der erste Entwurf legte einen TS-Zwilling des Snippets an — also die fünfte Kopie und die zweite in TypeScript, denn `app/utils/mediaKind.ts` hat längst einen, und zwar genau den, der entscheidet, was überhaupt je in diese Metafelder GESCHRIEBEN wird. Der Audit liest jetzt mit demselben Parser zurück, der schreibt. Die neue Kopie war zudem schwächer (Substring-Vergleich statt `new URL()`, keine ID-Validierung) und akzeptierte `https://notyoutube.com/watch?v=…` als YouTube-Video.
- **Ein Live-Storefront-Bug, den der Review dabei ausgegraben hat.** Liquids `split` ist Rubys und verwirft nachlaufende Leerstrings: `"https://youtu.be/" | split: 'youtu.be/' | last` liefert `"https://"`, die Schnittkette reduziert das auf `"https:"` — nicht blank. Das Snippet antwortete also `youtube|https:` und der Block emittierte `"embedUrl": "https://www.youtube.com/embed/https:"`; der Galerie-Block baute daraus einen kaputten iframe. Eine Video-ID enthält weder `:` noch `.`, und genau das prüft das Snippet jetzt. Der Test hatte diese Eigenschaft aus dem Docstring des Snippets zitiert — gegen die falsche Seite.

**Nicht verifiziert:** Die Wurzel-Abfrage `productVariants` wird sonst nirgends in dieser App benutzt; sie ist gegen keinen Live-Shop gelaufen. Der Ausfallmodus ist aber gutartig und getestet — eine unbekannte Abfrage liefert `null`, also „nicht geprüft", nie „nichts gefunden".

**Der Review-Durchgang danach (2026-08-19)** fand einen Befund, bei dem der Rat der App aktiv Schaden angerichtet hätte, plus neun kleinere:

1. **„liegt NUR in einer Galerie" wurde nie geprüft.** Der Block speist seinen Dedup-Satz aus den Produkt-Medien, bevor der Galerie-Durchlauf läuft — ein Video, das in beiden liegt, wird einmal aus der Medien-Schleife emittiert, mit dem automatischen Datum. Der Audit meldete es als „Datum fehlt", und das Befolgen dieses Rats ist eine **Regression**: `custom.video_upload_date` ist der produktWEITE Override und hätte den exakten `File.createdAt`-Stempel *jedes* Medien-Videos dieses Produkts durch ein geschätztes Datum ersetzt. Die Medien-Schlüssel werden jetzt abgezogen, genau wie im Block. Gleiche Klasse, kleiner: die 5-Videos-Grenze des Blocks wird ebenfalls modelliert.
2. **Kein `THROTTLED`-Handling.** Shopify meldet Drosselung als HTTP 200 mit einem Eintrag in `errors`, den der Transport nicht wiederholt — der Sweep gab beim ersten belegten Moment auf, also auf genau den Shops, die groß genug für Varianten-Galerien sind. Jetzt mit Backoff aus Shopifys eigenem `throttleStatus`.
3. **Draft- und archivierte Produkte** wurden als Defekt gemeldet — dieselbe Regel, der `catalog-readiness.service.ts` schon folgt.
4. Ein Produkt mit einem YouTube- **und** einem Vimeo-Video hörte vom Vimeo-Problem nichts (`vimeoOnly` verlangte `youtube === 0`).
5. `undefined` (nie geprüft) und `null` (Sweep abgelehnt) fielen in der UI zusammen; ein Shop, dessen Sweep jedes Mal scheitert, sah die Seite von vor der Funktion, für immer.
6. Der Befund trug keinen Zeitstempel — wer die Produkte repariert und nicht neu prüft, liest eine veraltete Liste als aktuellen Stand.
7. `scannedVariants === 0` ohne `capped` hätte als „keine gefunden" gerendert; jetzt ebenfalls `null`.
8. Fehlender `endCursor` bei `hasNextPage` hätte Seite eins zwanzigmal gelesen und 2000 geprüfte Varianten für einen Ein-Varianten-Shop gemeldet.
9. `VARIANTS_PER_PAGE` war tot (die Query hatte `first: 100` hart drin) und `handle` wurde geholt und nie gelesen.
10. Der Kommentar über Schritt 2 („liest den DB-Cache, nie eine Live-Seite") stimmte nicht mehr.
