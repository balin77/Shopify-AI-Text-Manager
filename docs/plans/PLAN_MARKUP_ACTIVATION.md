...
- **Ein bewusster Duplikat-Parser.** [external-video-url.shared.ts](../../app/services/seo/external-video-url.shared.ts) ist das TS-Zwilling von `snippets/cp-external-video.liquid`; Liquid kann kein TS aufrufen und umgekehrt. Genau diese Duplizierung war §3.3s Bug (drei Kopien, zwei davon ohne `youtube.com/shorts/`), deshalb hält [external-video-url.test.ts](../../tests/unit/external-video-url.test.ts) beide Seiten an derselben Tabelle fest und prüft, dass das Snippet dieselben Zweige trägt.
