...
**Weiterhin offen, als eigenes Stück Arbeit:** Die App **erkennt** den Fall nicht. `validateJsonLd` hat keinen Video-Zweig, der Katalog-Audit deckt nur Product/Collection/Article ab, und `custom.video_upload_date` wird serverseitig nirgends gelesen — ein Shop mit zwölf datumslosen Galerie-Videos bekommt kein Signal, nur den allgemeinen Hinweis. Ein echter Befund („3 Produkte haben Galerie-Videos ohne Datum", mit Deep-Link) bräuchte einen Sweep über zwei Varianten-Metafelder plus `custom.video_upload_date` und gehört in die Bucket-Form des Katalog-Audits. Lohnt sich, sobald ein Shop das Feature wirklich nutzt.
