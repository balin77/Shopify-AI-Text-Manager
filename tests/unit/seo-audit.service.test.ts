...
// ─── analyzeLocale (Translation SEO card — per-secondary-locale presence audit) ─

describe("analyzeLocale", () => {
  interface Row {
    id: string;
    title: string;
  }
  ...
  /** Hand-rolled db stub: id/title reads per type + a real-ish contentTranslation.groupBy
   * that actually applies the where clause (locale/resourceType/key/value filters), so
   * the tests exercise the same filtering analyzeLocale relies on instead of a canned
   * response. */
    ...
    return {
      ...
      contentTranslation: {
        groupBy: async ({ where }: any) => {
          ...
          const resourceTypes = new Set<string>(where.resourceType.in);
          const matching = translations.filter(
            (t) =>
              t.locale === where.locale &&
              resourceTypes.has(t.resourceType) &&
              keys.has(t.key) &&
              t.value !== "", // mirrors the real `value: { not: "" }` where-clause filter
          );
          const seen = new Set<string>();
          const rows: { resourceId: string; key: string }[] = [];
          for (const t of matching) {
            const k = `${t.resourceId}:${t.key}`;
            if (seen.has(k)) continue;
            seen.add(k);
            rows.push({ resourceId: t.resourceId, key: t.key });
          }
          return rows;
        },
      },
  ...
  it("computes translated/missing coverage per field independently", async () => {
    const products: Row[] = [
      { id: "gid-P1", title: "Product One" },
      { id: "gid-P2", title: "Product Two" },
    ];
    const translations: TranslationRow[] = [
      { resourceId: "gid-P1", resourceType: "Product", locale: "fr", key: "title", value: "Produit Un" },
      { resourceId: "gid-P1", resourceType: "Product", locale: "fr", key: "meta_title", value: "Meta FR" },
      // gid-P1's meta_description and everything for gid-P2 stays untranslated.
    ];
    const db = makeDb({ products, translations });

    const result = await analyzeLocale("shop.myshopify.com", "fr", { db: db, plan: "pro" });

    expect(result.locale).toBe("fr");
    expect(result.totalItems).toBe(2);

    expect(result.totals.title.translated).toBe(1);
    expect(result.totals.title.missingTotal).toBe(1);
    expect(result.totals.title.missing).toEqual([
      { type: "product", id: "gid-P2", title: "Product Two" },
    ]);

    expect(result.totals.metaTitle.translated).toBe(1);
    expect(result.totals.metaTitle.missingTotal).toBe(1);

    // No item has a translated meta_description -> both missing, independent
    // of the title/metaTitle results above.
    expect(result.totals.metaDescription.translated).toBe(0);
    expect(result.totals.metaDescription.missingTotal).toBe(2);
  });

  it("treats a translation row with an empty value as missing, not translated", async () => {
    const products: Row[] = [{ id: "gid-P1", title: "Product One" }];
    const translations: TranslationRow[] = [
      { resourceId: "gid-P1", resourceType: "Product", locale: "fr", key: "meta_title", value: "" },
    ];
    const db = makeDb({ products, translations });

    const result = await analyzeLocale("shop.myshopify.com", "fr", { db, plan: "pro" });
    expect(result.totals.metaTitle.translated).toBe(0);
    expect(result.totals.metaTitle.missingTotal).toBe(1);
    expect(result.totals.metaTitle.missing).toEqual([
      { type: "product", id: "gid-P1", title: "Product One" },
    ]);
  });

  it("caps the missing-item ref list at MAX_LOCALE_MISSING_ITEMS while keeping the true missingTotal", async () => {
    const total = MAX_LOCALE_MISSING_ITEMS + 5;
    const products: Row[] = Array.from({ length: total }, (_, i) => ({
      id: `gid-P${i}`,
      title: `Product ${i}`,
    }));
    const db = makeDb({ products, translations: [] }); // nothing translated

    const result = await analyzeLocale("shop.myshopify.com", "fr", { db, plan: "pro" });
    expect(result.totals.title.missingTotal).toBe(total);
    expect(result.totals.title.missing.length).toBe(MAX_LOCALE_MISSING_ITEMS);
  });

  it("respects plan-type gating like analyzeStore (free -> no articles/pages)", async () => {
    const db = makeDb({
      products: [{ id: "gid-P1", title: "P1" }],
      collections: [{ id: "gid-C1", title: "C1" }],
      articles: [{ id: "gid-A1", title: "A1" }],
      pages: [{ id: "gid-PG1", title: "PG1" }],
      translations: [],
    });

    const result = await analyzeLocale("shop.myshopify.com", "fr", { db, plan: "free" });
    // free = products + collections + onlineStoreExtras -> articles/pages excluded.
    expect(result.totalItems).toBe(2);
    expect(result.totals.title.missingTotal).toBe(2);
  });

  it("returns zeroed-out coverage (and skips the groupBy call) when there are no items at all", async () => {
    const db = makeDb({});
    const result = await analyzeLocale("shop.myshopify.com", "fr", { db, plan: "pro" });
    expect(result.totalItems).toBe(0);
    expect(result.totals.title).toEqual({
      translated: 0,
      total: 0,
      coveragePct: 0,
      missingTotal: 0,
  ...
  it("never reports a false 100% while items are still missing (rounds 199/200 down to 99, not 100)", async () => {
    const total = 200;
    const products: Row[] = Array.from({ length: total }, (_, i) => ({
      id: `gid-P${i}`,
      title: `Product ${i}`,
    }));
    const translations: TranslationRow[] = products.slice(0, 199).map((p) => ({
      resourceId: p.id,
    ...
    const result = await analyzeLocale("shop.myshopify.com", "fr", { db, plan: "pro" });
    expect(result.totals.title.translated).toBe(199);
    expect(result.totals.title.missingTotal).toBe(1);
    expect(result.totals.title.coveragePct).toBe(99); // Math.round(199/200*100) is 100, but 1 item is still missing
  });
});
