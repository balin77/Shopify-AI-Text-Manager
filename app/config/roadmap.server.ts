...
export const ROADMAP: RoadmapEntry[] = [
  ...
  },
  {
    ...
    status: "planned",
    area: "platform",
    title: "Managed-AI plan variants (Basic 19.90 / Pro 39.90 / Max 99.90), per-tier budget, margin guard",
    notes: "The commercial half of managed-ai-key. Shape: entitlements stay 4-valued, key source is a SECOND axis — no eight plans. Each paid tier gets a managed variant with its own Shopify subscription NAME and PRICE (getPlanFromSubscription resolves by name then price, so no two variants may share a price). Provisional 2026-09-17: surcharge 10/20/40 EUR against a monthly PROVIDER-cost budget of 1.50/3.00/6.00 EUR, i.e. a ~17.6% cost share after taking Shopify's revenue share at its worst case (15%, though it is 0% below $1M today). The guard is a TEST, not a promise: budget x 1.25 (FX + list-price + overshoot buffer) <= surchargeNet x 0.20, plus the ladder rule taster <= 0.25 x smallest paid budget — raising a budget or cutting a price without the other then fails the build. Every number is provisional until ai-usage-metering has measured the real average call; the table is built so one measured constant updates it. Model is PINNED in managed mode (cost control a customer can switch off is not cost control) and the current DEFAULT_MODELS are already stale (deepseek-chat retired 2026-07-24, gemini-2.0-flash-lite), which is harmless for BYO and an outage for managed. Deliberately not in v1: Shopify usage-based overage (appUsagePricing + cappedAmount) — the right instrument, a second line item, the first follow-up. PRICING_AND_LIMITS.md argues its limits from 'AI costs us nothing' — true for BYO, false for managed, and that document is qualified in the same change rather than left as a second, contradicting answer.",
    ref: "docs/plans/PLAN_MANAGED_AI_KEY.md",
  },
  {
    id: "white-label",
