...
export const ROADMAP: RoadmapEntry[] = [
  ...
  },
  {
    ...
    notes: "Phase 0 of the managed-key plan, SHIPPED 2026-09-20 and metering BYO traffic; every number in the pricing entry below stays provisional until it has run for 2-4 weeks. It captures each provider SDK's real usage object (which _executeAIRequestInner used to discard, leaving estimateTokens — a rate-limiter figure that charges a flat 8192 output tokens — as the only token number in the app), prices it through one micro-EUR table and writes AiUsageCounter. The charge sits in executeAIRequest, not askAI, because replayRequest (task recovery) bypasses askAI; the ledger key is (shop, period, source, provider, model, feature, estimated), because a key without those dimensions answers only 'what did this shop spend in total', which is not the question Phase 0 exists for — and Task, the only store that carried the feature, is deleted after 3 days. Four rules, each a direction of error: an absent usage object ESTIMATES (rounded up, with a flat allowance per image, since a vision call's tokens are not in the prompt string) and is keyed apart from measured rows so the average can be computed without it; an unknown model is priced at a STATED per-provider ceiling rather than at the table's own maximum (deriving it from the table under-priced Claude Opus 4 — a model in this app's own picker — by 3x while logging that it had been 'priced at the ceiling'); a provider that ANSWERED is charged even when we then reject the answer, which covers the sixteen empty-content guards and the finish_reason:length truncation that is the most expensive call shape there is, plus a worst-case charge for a call that loses the 120s timeout race; and the volume columns are BIGINT, because Int32 holds 2.1e9 tokens (~EUR 99 of input spend on the pinned managed default) and Postgres RAISES rather than clamping, which would have stopped a shop's whole row mid-period. Read it with npm run ai:usage. The rule that must ship WITH enforcement, not after it: a budget refusal inside the detached repair (stale-translation-sync.server.ts) must be an ABORT, never an entry in outcome.failed — mayPurge is `purgeOnPrimaryChange || autoTranslateExternalChanges`, i.e. always true on exactly the shops managed mode serves, so a refusal that looks like 'the AI could not deliver' sends translationsRemove and DELETES the merchant's storefront translations because our prepaid budget ran out, unrecoverably. Metering BYO too is not scope creep: it is the measurement the price ladder is derived from, and it answers for BYO shops what AI_PROVIDER_BALANCE_FEASIBILITY.md concluded no provider answers uniformly.",
    ref: "docs/plans/PLAN_MANAGED_AI_KEY.md",
  },
  {
    ...
    notes: "The commercial half of managed-ai-key. Shape: entitlements stay 4-valued, key source is a SECOND axis — no eight plans. Each paid tier gets a managed variant with its own Shopify subscription NAME and PRICE (getPlanFromSubscription resolves by name then price, so no two variants may share a price). Provisional 2026-09-17: surcharge 12/20/40 EUR against a per-BILLING-PERIOD PROVIDER-cost budget of 1.50/2.50/5.00 EUR, i.e. a ~14.7% cost share after taking Shopify's revenue share at its worst case (15%, though it is 0% below $1M today). The budgets are what the guard ALLOWS, not round numbers: with the buffer the effective ceiling is 16% of net, and the plan's first draft (1.50/3.00/6.00 against 10/20/40) read as 17.6% and failed its own test — which is the point of having it as a test. Basic is 21.90 and not 19.90 because 19.90 is PRO's price today and getPlanFromSubscription falls back to a price match: a renamed Basic+AI subscription would have resolved to Pro. Four things the first draft had no answer for, each found in review and each free money for somebody: the 7-day trial grants a full budget for zero revenue (and redact restores eligibility), a partner dev store holds a Shopify-verified test:true subscription that charges nothing (owner's rule 2026-09-19: such a shop gets the Free tier's AI allowance — the one-time taster, never a period budget — and NOTHING else about it changes. ONLY AI, and for a structural reason rather than a threshold: image operations, PageSpeed runs and IndexNow submissions are all spent by a human doing one thing at a time, bounded by how fast somebody can click, while managed AI is the one budget a webhook storm or a nightly sweep can spend unattended in the thousands. BYO stays unlimited (it costs us nothing and is how a dev store tests AI at all) and ENTITLEMENTS stay on the subscribed plan — substituting the plan wholesale would reach getSyncScope/planCacheCleanup, where a 'free' answer deletes cached products past 50 on the owner's own test store; capping only the budget means no substitution exists to leak there. Detected by THREE signals, not two: partnerDevelopment, a test:true subscription, and resolveDevPlanMode !== null — the third was missed in the first cut and is the one that matters, because checkAndSyncSubscription consults getDevForcedPlan FIRST and returns early, so a devForcedPlan shop has no subscription object at all and, on a real store, partnerDevelopment false. The answer is PERSISTED three-valued at sync time rather than looked up per call: isDevStore's catch returns false, so 'could not tell' and 'not a dev store' are the same value, and the resolver runs per AI request on detached paths that hold no admin client), a calendar-month counter against EVERY_30_DAYS billing grants two budgets to a sign-up on the 31st (29.4% cost share, 1.47x the guard) and makes upgrade-spend-cancel a ~3.87 EUR/shop loop, and a refunded charge does not refund the spent tokens. The guard is a TEST, not a promise: budget x 1.25 (FX + list-price + overshoot buffer) <= surchargeNet x 0.20, plus the ladder rule taster <= 0.25 x smallest paid budget — raising a budget or cutting a price without the other then fails the build. Every number is provisional until ai-usage-metering has measured the real average call; the table is built so one measured constant updates it. In managed mode the AI-keys tab is HIDDEN rather than read-only (owner, 2026-09-19), and the stored keys must SURVIVE it. Review corrected the mechanism: every other settings tab posts its own narrow actionType and none touches the key columns, so hiding the tab removes the only writer and the 'next save nulls them' scenario cannot happen. The real hazard is structural — the key columns are written by the unnamed else CATCH-ALL, encryptApiKey(undefined) returns null exactly like encryptApiKey(''), and the same branch 400s when preferredProvider/appLanguage are absent. Fix is a named saveAiKeys action plus a 400 for unknown actionTypes. It also surfaced a LIVE bug with no managed mode involved: the AI tab's payload omits seoTitleSuffix(Enabled) while the catch-all writes them, so saving the AI tab clears the merchant's SEO title suffix today. Hiding the tab is also not self-contained — app.tsx raises a permanent app-wide 'add an API key' warning linking to the hidden tab, ThemeContentDomainPage gates a feature on the six has*ApiKey booleans, and the Settings loader ships all six DECRYPTED keys to the browser, so managed mode needs a 'has a working AI source' predicate, a one-line 'N stored keys' row with a Delete control (GDPR/App-Review: a merchant must be able to erase a credential they gave us) and a loader that stops decrypting. The key columns are written only by the save that owns them, preferredProvider and selectedModel survive too (back to my own key means back to the merchant's own setup), and the mode switch stays visible because the way back must never be the hidden thing. TWO models, two PROVIDERS, both pinned and neither merchant-selectable (owner, 2026-09-19): default gpt-5-nano, failover claude-haiku-4-5 — different company, different status page, vision and no-training-by-default on both. Haiku is ~14x nano blended, and that one number drives every failover rule: the merchant's budget is debited at the DEFAULT price whatever ran (an outage must not shrink what they bought), we absorb the difference, and a global failover budget bounds it (a full day of everyone on the fallback is ~47% of a month's provider budget). Failover triggers are a short list (5xx, timeout, connection, model-not-found, our own 401; a 429 only after the queue's retries) and the exclusions are where the money is — never on input-too-long, a content refusal or a malformed 400, where the second provider returns the same answer and we pay twice. A circuit breaker on a failure RATE (not 'N consecutive' — dispatch is concurrent and settles out of order) whose half-open probe is the next real managed call, sound only because production is a single instance. Four rails review added and the first cut lacked: a PER-SHOP failover ceiling, because the triggers are shop-reachable and the breaker is global, so one shop could flip everyone onto the 14x model at 1x prices and drain the shared pool; EVERY managed refusal (managedUnavailable, kill switch, breaker-open) must reach the detached repair as an ABORT like budgetExceeded, or the pool emptying mid-outage deletes every translation of every product a supplier feed touched; the rate-limit bucket must be chosen at DISPATCH, not enqueue, and the fallback account's real limits are a precondition (the queue's default Anthropic bucket would admit ~4 calls a minute for the whole app); and the failover needs a CONTRACT gate, because the batch paths throw on a count mismatch and on the repair path a throw becomes a deletion. Consent names BOTH providers from the first managed call: a sub-processor that first appears during an outage is not one anybody consented to. Cost control a customer can switch off is not cost control and several model ids in the repo are already stale (DEFAULT_MODELS carries deepseek-chat and gemini-2.0-flash-lite; CURATED_MODELS adds gemini-1.5-*, gpt-4-turbo, o3-mini and an invalid claude-opus-4-0 id) — harmless for BYO, an outage for a PINNED managed model. Retirement dates are external facts, re-checkable but not verifiable from the repo. Deliberately not in v1: Shopify usage-based overage (appUsagePricing + cappedAmount) — the right instrument, a second line item, the first follow-up. Provider terms checked 2026-09-18 and they permit this shape: building an application for end users is granted, reselling account/API access is not — so the offer is worded as 'a plan with AI included, fair-use volume', never as a token quantity, which is also the unit the codebase cannot hold stable. Production runs a SINGLE instance (owner, 2026-09-18), which is what makes the overshoot bound and the in-memory queue/rate-window/ticks sound — load-bearing, and the day it is scaled to two every one of them doubles or races. PRICING_AND_LIMITS.md argues its limits from 'AI costs us nothing' — true for BYO, false for managed, and that document is qualified in the same change rather than left as a second, contradicting answer.",
    ref: "docs/plans/PLAN_MANAGED_AI_KEY.md",
  },
  {
    id: "white-label",
