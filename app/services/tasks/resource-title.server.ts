...
} from "./task-resource-kind.shared";

/**
 * The GID -> kind rule moved to `task-resource-kind.shared.ts` when the Tasks
 * page learned to link a row into THIS app's editor: that link needs the same
 * answer in the browser, and this module cannot be imported there. Re-exported
 * rather than re-declared, so there stays exactly one map.
 */
export { taskResourceKind, type TaskResourceKind };
