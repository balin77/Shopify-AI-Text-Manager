...
export async function exportCustomerData(
  ...
  console.log(`📦 [GDPR] Exporting data for customer ${customer.id} from shop ${shop_domain}`);
...
export async function redactCustomerData(
  ...
  console.log(`🗑️  [GDPR] Redacting data for customer ${customer.id} from shop ${shop_domain}`);
  ...
  console.log(`✅ [GDPR] Redacted ${deleted.count} sessions for customer ${customer.id}`);
}
...
export async function redactShopData(
  ...
  console.log(`🗑️  [GDPR] Redacting ALL data for shop ${shop_domain}`);
  ...
  await db.$transaction(async (tx) => {
    ...
    console.log(`  ✅ Deleted ${aiSettingsDeleted.count} AI settings`);

    // 3. Delete AI instructions
    const aiInstructionsDeleted = await tx.aIInstructions.deleteMany({
      where: { shop: shop_domain },
    });
    console.log(`  ✅ Deleted ${aiInstructionsDeleted.count} AI instructions`);
    ...
    console.log(`  ✅ Deleted ${productsDeleted.count} products (with cascading relations)`);
    ...
    console.log(`  ✅ Deleted ${menusDeleted.count} menus`);
    ...
    console.log(`  ✅ Deleted ${webhookLogsDeleted.count} webhook logs`);
  });

  console.log(`✅ [GDPR] Successfully redacted ALL data for shop ${shop_domain}`);
}
...
export async function logGDPRRequest(
  ...
  console.log('📋 [GDPR LOG]', JSON.stringify(logEntry, null, 2));
