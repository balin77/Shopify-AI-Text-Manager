 ...
 *
 * This route is called by Shopify when blog articles change.
 * It syncs the article data to our local database for fast access.
 */
export const action = async ({ request }: ActionFunctionArgs) => {
  console.log("🎣 [WEBHOOK] === ARTICLE WEBHOOK RECEIVED ===");
  ...
  try {
    ...
    console.log(`[WEBHOOK] Article ID: ${articleId}`);
...
async function processWebhookAsync(
  ...
  try {
    ...
    if (topic === "articles/create" || topic === "articles/update") {
      console.log(`[WEBHOOK-ASYNC] Syncing article: ${articleId}`);
      await syncService.syncArticle(articleId);
    } else if (topic === "articles/delete") {
      console.log(`[WEBHOOK-ASYNC] Deleting article: ${articleId}`);
