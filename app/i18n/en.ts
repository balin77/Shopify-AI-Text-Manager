...
export const en: Translation = {
  ...
  directTranslations: {
    ...
    aboutDetails: "How it works: your theme app embed (\"ContentPilot — Direct translation\") walks every text node on the storefront, looks it up in your item dictionary, and replaces it live with the translation for the current locale. This happens client-side — perfect for widget labels and short app strings, but NOT indexed by search engines. For anything SEO-relevant (product titles, descriptions, theme strings) keep using the regular Shopify field translations. Apps that render their content in cross-origin iframes (e.g. Loox reviews) can't be reached by the embed — that's a hard browser boundary, not an app limitation. Workflow tip: enable the embed under \"Customize theme → App embeds → ContentPilot — Direct translation\" (nothing happens on the storefront otherwise). The capture tool inside the theme editor (toggled in the embed settings) lets you click any rendered text to add it as an item instead of typing it by hand. The \"Collect texts from the storefront\" toggle additionally records every untranslated string real visitors see — you can review them in the \"Found texts\" modal and adopt them as items in one go.",
  ...
  settings: {
    ...
    themeSetupNote: "After each activation, save your theme. Both blocks belong to the same ContentPilot Storefront extension.",
