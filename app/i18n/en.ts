...
export const en: Translation = {
  ...
  settings: {
    ...
    autoTranslateExternalChangesHelp: "When a primary-language text changes — in the Shopify admin, in another app, through an import, or here in ContentPilot — the AI re-translates it instead of only deleting the outdated translation. For products and collections that happens automatically on the next sync. For everything else — pages, blogs, articles, policies, options, metafields, metaobjects, theme texts, alt texts and menu titles — it happens when you save in the editor; if the text was changed elsewhere, on the next reload of that item. In the bulk editor translations are still only deleted. This spends AI credit without you being present. URL handles are excluded and are still only deleted.",
    autoTranslateExternalChangesPlanHint: "Available from the {plan} plan.",
    translationPurgeSupersededNote: "Not needed while texts are re-translated automatically: for products and collections on the next sync, for everything else when you save in the editor — pages, blogs, articles, policies, options, metafields, metaobjects, theme texts, alt texts and menu titles. There the outdated text is replaced rather than deleted. In the bulk editor deletion continues: a 200-row save there would start 200 unattended AI runs.",
