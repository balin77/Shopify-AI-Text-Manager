...
export const en: Translation = {
  ...
  seo: {
    ...
    structuredDataPage: {
      ...
      schemaVideoNote: "Video is produced on the storefront — from your product media AND from YouTube links that only live in a variant gallery — so it does not appear in the preview below. Deduplicated per product: one video attached to twelve variants stays one entry. For product media, the upload date Google asks for is fetched from the Shopify API during the product sync and stored in the \"custom.video_upload_dates\" metafield, which the storefront reads. A gallery link has no file behind it and therefore no automatic date: without the \"custom.video_upload_date\" metafield (your own value, which always wins) the property is omitted rather than invented — the video rich result then stays incomplete. Vimeo links from a gallery are skipped entirely: the rich result needs a thumbnail and none can be derived from a Vimeo link. FAQ (FAQPage) joins in once you enable the switch in the theme editor and fill the \"custom.faq\" metafield — both are deliberately off by default.",
      ...
      activation: {
        ...
        socialSwitchesTitle: "\"Open Graph / Twitter\" app block",
        socialSwitchesBody: "This block has just ONE switch in the theme editor but sets nine tags. Because your theme typically sets og:title and og:image itself while leaving twitter:* alone, each tag is listed separately — the switch is only safe while none of them is red.",
        ...
        socialOriginUnknownHint: "This crawl cannot say which social tags came from this app — either the Open Graph embed is OFF, or the crawl predates the origin marker in the block. That marker ships with this update: after the next extension release and a fresh crawl, a recommendation appears here.",
        title: "Switch by switch: what may go on?",
        intro: "Every switch of the \"JSON-LD Structured Data\" app block, next to what step 1 found. Worth knowing: our product block deliberately uses the same @id as your theme's schema, because it is meant to REPLACE it. Run both and Google merges them into one node, with every field both sides deliver present twice — that is not duplicate markup, it is invalid markup.",
        noCrawl: "No completed site crawl yet. Without a measurement there is no recommendation here — run step 1 first.",
        notMeasured: "The last crawl predates this analysis and captured no JSON-LD data. Without a measurement there is no recommendation here — start a new crawl.",
        basis: "Based on: crawl from {time}, {pages} served pages checked.",
        originUnknownHint: "This crawl cannot say which markup came from this app. There are two causes and the data cannot tell them apart: either the JSON-LD embed is OFF in your theme (in which case your theme is serving what you see below), or the crawl predates the origin detection. While that holds, the same applies to every switch below: do not newly switch it on, and if it is already on, change nothing until a fresh crawl resolves it.",
        switchesTitle: "Switches in the \"JSON-LD Structured Data\" app block",
        defaultOn: "Default: on.",
        defaultOff: "Default: off.",
        repeatableCaveat: "Several blocks of this type on one page are normal — three product videos are three VideoObjects. Duplication is therefore not measurable here; this field deliberately stays without a verdict rather than pretending to be \"checked and clean\".",
        themeHint: "You switch your theme's own markup off in the theme editor, not here: this app never touches theme code it does not own. It names the conflict — the decision stays yours.",
        switches: {
        ...
        verdictLabels: {
          ...
          free: "Safe to switch on",
          appOnly: "Running cleanly",
          foreignOnly: "Your theme already serves it",
          mixed: "Inconsistent",
          originUnknown: "Origin unclear — do not switch on",
          repeatableUnjudged: "Not judgeable",
          duplicateApp: "Duplicated — fixable here",
          duplicateForeign: "Duplicated — not ours",
        },
        verdicts: {
          unknown: "No measurement, no recommendation. Run step 1 and a verdict appears here.",
          free: "No crawled page serves this type from any source. You can switch it on safely.",
          appOnly: "Served on {pages} pages, exactly once per page and by this app. That is the intended state — nothing to do.",
          foreignOnly: "Served on {pages} pages, none of them by this app — so by your theme or another app. Switching this on would put the type on the page twice. Leave it off, or remove the other source first.",
          mixed: "Served on {pages} pages, {appPages} of them by this app; on the rest it comes from your theme or another app. Decide which source to keep — otherwise what Google sees depends on the page template.",
          originUnknown: "Served on {pages} pages — but whether one of the copies is ours cannot be determined (embed off, or crawl older than the origin detection). So do not newly switch this on: the type is already there and a second copy would be one too many. If it is already on, leave it until a fresh crawl shows the origin.",
          repeatableUnjudged: "Served on {pages} pages, {appPages} of them carrying blocks from this app. Whether your theme marks up the same videos as well cannot be measured.",
          duplicateApp: "This type appears more than once on {duplicatePages} pages — on {appIsOneCopy} of them one of the copies is ours. Switch this off and only your theme's version remains, which clears the error. The reverse works too: one of the two sources has to go, and which one is your call.",
          duplicateForeign: "This type appears more than once on {duplicatePages} pages, but none of the copies came from this app. Our switch does not fix it — the sources are your theme and/or another app. Search the page source for \"application/ld+json\" and compare the blocks.",
        },
      },
