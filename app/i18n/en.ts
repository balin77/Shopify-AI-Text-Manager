...
export const en: Translation = {
  ...
  settings: {
    ...
    autoTranslateExternalChangesHelp: "When a primary-language text changes — in the Shopify admin, in another app, through an import, or here in ContentPilot — the AI re-translates it instead of only deleting the outdated translation. For products and collections that happens automatically on the next sync. For pages, blogs, articles and policies it happens right away when you save them in the editor — if the text was changed elsewhere, on the next reload of that item. In the bulk editor those four are still only deleted. This spends AI credit without you being present. URL handles are excluded and are still only deleted.",
    autoTranslateExternalChangesPlanHint: "Available from the {plan} plan.",
    translationPurgeSupersededNote: "Not needed for products and collections while texts are re-translated automatically — nor for pages, blogs, articles and policies when you edit them in the editor: there the outdated text is replaced rather than deleted. In the bulk editor, and for metaobjects, theme texts, options, metafields and alt texts, deletion continues: nothing re-translates those automatically yet, and a translation without its source must not stay live.",
