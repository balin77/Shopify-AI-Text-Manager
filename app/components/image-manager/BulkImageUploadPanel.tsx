...
function buildVariantKey(
  ...
  const opts = variant.selectedOptions.length > 0
    ...
    : variant.title.split(" / ").map(v => ({ name: "", value: v, handle: null, metaobjectGid: null, optionValueGid: null }));
              ...
              {variants.length > 0 && (
                <BlockStack gap="200">
                  {variants.map(v => {
                    const opts = v.selectedOptions.length > 0
                      ? v.selectedOptions
                      : v.title.split(" / ").map(val => ({ name: "", value: val, handle: null, metaobjectGid: null, optionValueGid: null }));
                    return (
